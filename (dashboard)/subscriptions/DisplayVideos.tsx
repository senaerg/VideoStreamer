...
import { fetchSubscribedVideos } from "@/lib/actions/stream";
...
const fetcher = cache(async (args: IFethArgs): Promise<AppVideo[]> => {
  const response = await fetchSubscribedVideos(args.userId, args);
  if (!response.data) {
...
const DisplayVideos = ({
  ...
  const getKey = (pageIndex: number, previousPageData?: AppVideo[]) => {
    ...
    return { type: "subscriptions", limit: PAGE_SIZE, skip: pageIndex * PAGE_SIZE, userId };
  };
