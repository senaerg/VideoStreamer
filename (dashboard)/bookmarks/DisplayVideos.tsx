...
import { fetchBookmarkedVideos } from "@/lib/actions/stream";
...
const fetcher = cache(async (args: IFethArgs): Promise<AppVideo[]> => {
  const response = await fetchBookmarkedVideos(args.userId, args);
  if (!response.data) {
...
const DisplayVideos = ({
  ...
  const getKey = (pageIndex: number, previousPageData?: AppVideo[]) => {
    ...
    return { type: "bookmarks", limit: PAGE_SIZE, skip: pageIndex * PAGE_SIZE, userId };
  };
